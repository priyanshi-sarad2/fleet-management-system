...
- **CORS.** Cross-Origin Resource Sharing rules tell the browser which other **origins (domains)** are allowed to call the bucket directly from a web page. Only needed when a page served from one domain fetches objects straight from S3 on a different domain — **not** needed when a CDN (CloudFront) serves the objects, or for backend-only buckets.

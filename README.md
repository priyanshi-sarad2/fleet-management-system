...
The three backend microservices run as containers on EKS. The **Angular webapp is different** — once it's built it's just a bundle of static files (HTML, JS, CSS, images). Static files don't need a pod, a JVM, or a Node server to run; they only need somewhere to be stored and something to serve them quickly over HTTPS. So instead of running it in the cluster, the frontend is hosted as a static site on **Amazon S3** and served through **Amazon CloudFront**.

## Why S3 + CloudFront (and not a pod)

- **No server to run or scale** — `ng build` produces a `dist/` folder of static assets. S3 just stores them; there's no container to keep alive, patch, or autoscale.
- **Fast, global delivery** — CloudFront is a CDN: it caches the site at edge locations close to users, so load times are low everywhere instead of every request travelling to one region.
- **HTTPS for free** — CloudFront terminates TLS using an ACM certificate, so the site is served securely over `https://` with a custom domain.
- **Cheap** — you pay for storage + data transfer, which for a static SPA is a tiny fraction of running an always-on pod/load balancer.
- **Secure by default** — the S3 bucket stays **private**; only CloudFront is allowed to read from it (see OAC below). Users never hit S3 directly.

## The architecture

```text
Browser ──HTTPS──> CloudFront (CDN, edge cache, TLS, security headers)
                        │  Origin Access Control (OAC, SigV4-signed reads)
                        ▼
                 S3 bucket (PRIVATE)  ← holds the built Angular files (index.html, JS, CSS)
```

CloudFront is the only public entry point. It pulls the files from a **private** S3 bucket and serves them to browsers, adding caching, HTTPS and security headers along the way.

## How it's wired (Terraform)

This is all defined as code under `Infrastructure/` using a reusable `static-cloudfront` module (plus `s3` and `s3-bucket-policy` modules):

- **Private S3 bucket** — stores the built site. Public access is blocked; the bucket is not a website endpoint on its own.
- **Origin Access Control (OAC)** — a CloudFront identity that signs its requests to S3 with SigV4 (`signing_behavior = "always"`). This is the modern replacement for the older Origin Access Identity.
- **S3 bucket policy** — only allows the principal `cloudfront.amazonaws.com` to `s3:GetObject`, and only when the request comes from *this* distribution (`Condition: AWS:SourceArn == <distribution ARN>`). That's what keeps the bucket private while still letting CloudFront read it.
- **`default_root_object = index.html`** — so visiting the root domain serves the app's entry point.
- **HTTPS everywhere** — `viewer_protocol_policy = "redirect-to-https"`, an ACM certificate on the distribution, and `minimum_protocol_version = "TLSv1.2_2021"`.
- **Security headers policy** — CloudFront attaches a response-headers policy (Content-Security-Policy, HSTS, `X-Content-Type-Options`, `X-Frame-Options: SAMEORIGIN`, Referrer-Policy, XSS protection, Permissions-Policy) so the frontend ships with sensible security headers.
- **SPA deep-link fix** — an Angular SPA does its own client-side routing, so a direct hit to `domain.com/sign-in` is not a real file in S3 and would return `403`. A `custom_error_response` maps that error to `/index.html` with an HTTP **200**, letting the Angular router take over and render the right page.

## Build & deploy flow

Deploying a new version of the frontend is just "build, upload, invalidate":

```bash
# 1. Build the production bundle
npm ci
npm run build            # ng build --prod  ->  outputs to dist/

# 2. Upload the static files to the S3 bucket
aws s3 sync dist/ s3://<webapp-bucket-name>/ --delete

# 3. Invalidate the CloudFront cache so users get the new files immediately
aws cloudfront create-invalidation \
  --distribution-id <distribution-id> \
  --paths "/*"
```

`--delete` removes stale files that are no longer in the new build, and the cache invalidation forces CloudFront's edge caches to fetch the fresh version instead of serving the old cached one.

> **Note:** the webapp still talks to the backend over its public API endpoint (the api-gateway exposed via Ingress/ALB). Only the *static frontend* lives in S3 + CloudFront; the live data (vehicle positions over REST/WebSocket) still comes from the microservices running on EKS.

########    Static webapp hosting: S3 (private) + CloudFront (OAC)    ########
# NOTE: needs the AWS account to be verified for CloudFront before apply will succeed.

# 1) One private S3 bucket per static web app (stores the built site)
module "webapp_s3" {
  ...
  app         = each.key
  bucket_name = "${each.key}-${var.project_name}-${var.env}"
}

# 2) CloudFront distribution per app (private S3 origin via Origin Access Control)
